...
class minLine_cls:
    def exe_minLine(self, st_dd, inte_lowhigh, dataset, long, minute):
        ##--------------------------------------------------------------------------------------------------##
        ## 저점 구하기 ##
        ...
        lowpoint_minute = []
        lowpoint_touch1 = []
        lowpoint_touch1_price = []
        lowpoint_jiji = []
        lowpoint_touch2 = []
        lowpoint_touch2_price = []
        lowpoint_position = []
        lowpoint_standard = []
        lowpoint_before = []
        lowpoint_after = []
        ...
        for i in range(len(lowpoint_data_long)) :
            ...
            # 기준가
            standardprice = (lowhigh + lowlow + lowclose) / 3
            
            # 전수
            cnt_bf = lowpoint_data_long.iloc[i]['전수']
            
            # 후수
            cnt_af = lowpoint_data_long.iloc[i]['후수']
            ...
            lowpoint_touch1.append("없음")
            lowpoint_jiji.append("없음")
            lowpoint_touch2.append("없음")
            lowpoint_position.append("없음")
            lowpoint_touch1_price.append("없음")
            lowpoint_touch2_price.append("없음")
            lowpoint_standard.append(standardprice)
        ...
        lowpoint_grape_data_long = pd.DataFrame({
                                                ...
                                                '거래량' : lowpoint_volume_long, '구분' : lowpoint_minute, '기준가' : lowpoint_standard,
                                                '전수' : lowpoint_before, '후수' : lowpoint_after,
                                                '첫터치' : lowpoint_touch1, '지지저항' : lowpoint_jiji, '첫가격' : lowpoint_touch1_price,
                                                '둘터치' : lowpoint_touch2, '포지션' : lowpoint_position, '둘가격' : lowpoint_touch2_price
                                                 })
