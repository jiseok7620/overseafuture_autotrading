...
# 준비사항
# F:\JusikData\short_invest\test에 ----------- result, mini_hangseng1r 빈 파일이 있어야함
# 나스닥은 6~7만 쉼
...
class all_cls:
    ...
    def exe_all(self):
        ...
        if os.path.isfile('F:/JusikData/short_invest/test/mini_hangseng_lh1m.xlsx') :
            dataset_1m_ago = pd.read_excel('F:/JusikData/short_invest/test/mini_hangseng_lh1m.xlsx', engine='openpyxl')
        else :
            #### 과거 최대선, 최저선 구하기 ####
            # 전일 데이터 자르기
        ...
        for i in dataset_now.index :
            ...
            # 최저선, 최대선 데이터 가져오기
            minmaxLine_repeat_cls.exe_minmaxLine_repeat(self, inte_lowhigh_1m, inte_df_1m, 10) # 숫자 = 시작시점 이후 데이터 전후수
