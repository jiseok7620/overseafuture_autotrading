...
class all_cls:
    def exe_all(self):
        ...
        # 저장된 선물 데이터를 가져오기
        dataset = pd.read_excel('F:/JusikData/short_invest/mini_hangseng5.xlsx', engine='openpyxl')
        ...
        low_df = pd.DataFrame(columns=('일시','시가','고가','저가','종가','거래량','최저수'))
        ...
        while True:
            ...
            ### 3단계 : 데이터프레임에서 최저선 데이터 만들기 ###
            # 저점 구하기
            for i in range(count_ind) : 
                if count_ind == 0:
                    break
                if inte_df.iloc[count_ind]['저가'] >= inte_df.iloc[count_ind-i-1]['저가']:
                    break
                elif inte_df.iloc[count_ind]['저가'] < inte_df.iloc[count_ind-i-1]['저가']:
                    low_count_bf += 1
            ...
            if low_count_bf >= 10 :
                low_bf_arr.append(dd)
                low_bf_arr.append(open)
                low_bf_arr.append(high)
                low_bf_arr.append(low)
                low_bf_arr.append(close)
                low_bf_arr.append(td)
                #low_bf_arr.append(up_tail)
                #low_bf_arr.append(body)
                #low_bf_arr.append(down_tail)
                #low_bf_arr.append(leng_bar)
                low_bf_arr.append(low_count_bf)
            ...
            #########################################################################################################################
            # 고점 구하기
            for m in low_df.index:
                low_ind = inte_df[inte_df['일시']==low_df.iloc[m]['일시']].index[0]
                
                for n in range(low_ind+1, len(inte_df)):
                    if n == len(inte_df)-1:
                        break
                    if inte_df.iloc[low_ind]['저가'] >= inte_df.iloc[n]['저가']:
                        break
                    elif inte_df.iloc[low_ind]['저가'] < inte_df.iloc[n]['저가']:
                        low_count_af += 1
                
                low_af_arr.append(low_count_af)
                ### 카운트 초기화 ###
                low_count_af = 0
                
            ### 데이터프레임에 리스트를 열로 추가하기 ###
            low_df['최대수'] = low_af_arr
            ...
            low_data = low_df[low_df['최대수'] >= 10]
            low_data = low_data.reset_index(drop=True)
            
            # 첫번째 터치 구하기
            touch1_data_arr = []
            touch_date = '없음'
            
            for i in low_data.index: 
                low_ind2 = inte_df[inte_df['일시']==low_data.iloc[i]['일시']].index[0]
                for j in inte_df.index:
                    if j > low_ind2 :
                        if inte_df.iloc[j]['종가'] <= low_data.iloc[i]['저가']:
            ...
            for i in low_data.index: 
                for j in inte_df.index:
                    if low_data.iloc[i]['첫터치'] == '없음' :
                        break
                    else :
                        low_ind3 = inte_df[inte_df['일시']==low_data.iloc[i]['첫터치']].index[0]
                        if j > low_ind3 :
                            if inte_df.iloc[j]['종가'] >= low_data.iloc[i]['저가']:
            ...
            low_data['둘터치'] = touch2_data_arr
            low_data['진입가'] = touch2_price_arr
            
            #########################################################################################################################
            # 진입, 청산, 손절 해보기
            # 진입 = 둘터치 종가, 청산 = 진입에서 손절사이의 최대값 or 10분 내 최대값, 손절 = 첫터치에서 둘터치 사이의 최소값
            final_data = low_data[low_data["둘터치"] != "없음"]
            ...
            for i in final_data.index:
                ...
                stop_loss = inte_df[touch1_ind:touch2_ind]['저가'].min()
            ...
            if len(final_data) != len_fi : 
                ...
                print(dataset[ind+1:ind+21])
                
                len_fi = len(final_data)
            
            #########################################################################################################################
            ### 데이터프레임에서 최대수 열 없애기
            low_df = low_df.drop(['최대수'], axis=1)
            #print(count_ind)
    def min_line(self):
        pass
